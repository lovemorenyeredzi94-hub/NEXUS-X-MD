...
async function startpairing(kingbadboiNumber) {
    ...
    if (pairingCode && !state.creds.registered) {
        ...
        setTimeout(async () => {
            try {
                let code = await bad.requestPairingCode(phoneNumber, 'NEXUSXMD');
    ...
    bad.ev.on("connection.update", async (update) => {
        ...
        if (connection === "close") {
            ...
            try {
                ...
                console.log(chalk.green.bold(`🎉 𓆩 ☠︎︎ 𝙽𝙴𝙳𝚄𝚂-𝚇 𝙼 ☠︎︎online: ${kingbadboiNumber}`));
